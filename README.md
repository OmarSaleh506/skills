...
| **[shop-scout](./skills/shop-scout/SKILL.md)** | Any agent | Comparison-shops any product (or a whole shopping list) across online stores: finds sellers, ranks by **effective price** (price − coupon + shipping), judges each seller **Trusted/Mixed/Risky**, and hunts working **coupons** (incl. first-order & social-media) — then prints per-item tables, a best-buy pick, and a consolidated cart with a single-store bundle tip. Auto-detects a scraping backend: self-hosted Firecrawl (recommended, no account/quota) → Firecrawl cloud → built-in fetch (best-effort). Saudi + global store lists; configurable scope. |
